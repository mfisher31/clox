...
static Value clockNative (int n, Value* args) {
    (void) n;
    (void) args;
...
static bool bindMethod (ObjClass* klass, ObjString* name) {
    ...
    if (! tableGet (&klass->methods, name, &method)) {
        runtimeError ("Undefined class property '%s'", name->chars);
        return false;
    }
...
InterpretResult interpret (const char* code) {
    ObjFunction* function = compile (code);
    if (function == NULL)
