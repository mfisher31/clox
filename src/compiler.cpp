...
static int resolveLocal (Compiler* compiler, Token* name) {
    for (int i = compiler->localCount; --i >= 0;) {
...
/** returns the stack index of the new upvalue. */
...
static int resolveUpvalue (Compiler* compiler, Token* name) {
    // p 472. explains nullptr enclosing means compiler of the root function.
...
static void namedVariable (Token name, bool canAssign) {
    ...
    if (arg != -1) {
        ...
        arg   = (uint8_t) identifierConstant (&name);
        getOp = OP_GET_GLOBAL;
        setOp = OP_SET_GLOBAL;
    }
...
ParseRule rules[] = {
    ...
    { NULL, dot, PREC_CALL },      // 06 [TOKEN_DOT]
...
ObjFunction* compile (const char* code) {
    initScanner (code);
    Compiler compiler;
